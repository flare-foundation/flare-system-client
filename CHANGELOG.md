...
## [v1.2.0](https://github.com/flare-foundation/flare-system-client/tree/v1.2.0) - YYYY-MM-DD

### Added

- Support for the Relay contract cutover via a `[relay_cutover]` section: `address` (env `RELAY_CUTOVER_CONTRACT_ADDRESS`) and `starting_reward_epoch` (env `RELAY_CUTOVER_STARTING_REWARD_EPOCH`). From that epoch on, signing policies, finalizations and the already-relayed check use the new Relay; the switch-over voting round comes from the epoch's signing policy. Unset keeps the current behaviour; a half-set schedule fails startup.
- The random number and Merkle proof the new Relay's `relay()` requires are appended to the random protocol's finalization, taken from the new `finalizationData` field (`0x` hex, the value followed by the proof nodes) of that protocol's `submitSignatures` response. Missing or malformed data is logged and the round is not sent; with a cutover scheduled, the finalizer requires that protocol to be configured.
- The signing-policy listener selects the awaited policy by reward epoch instead of by log position, which across the cutover could pick the wrong contract's policy.

### Changed

- A prolonged reward epoch's finalization threshold is the signing policy's threshold scaled by the Relay's `thresholdIncreaseBIPS`, as the contract computes it, instead of 60% of total weight.
...
- The submitter rejects a provider message whose voting round or protocol id differ from the ones requested. `SignatureSubmitterDataVerifier` now takes both.
- Submitted signatures are canonicalized before they count: a high-`s` signature is normalized (logged at warning), an invalid `v`, `r` or `s` is rejected — either form makes the new Relay revert the whole `relay()`.
- The new Relay's custom-error reverts are decoded, so `AlreadyRelayed()` is treated as non-fatal and a reverting `eth_estimateGas` reports its reason.
