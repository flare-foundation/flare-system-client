...
- Support for the Relay contract cutover. A `[relay_cutover]` config section names the new Relay address (`address`, env `RELAY_CUTOVER_CONTRACT_ADDRESS`) and the first reward epoch it serves (`starting_reward_epoch`, env `RELAY_CUTOVER_STARTING_REWARD_EPOCH`); up to that epoch everything is read from and sent to the configured Relay exactly as before, and from it on to the new one. The voting round the switch takes effect on is not configured — a reward epoch's start can be delayed, so it is only fixed once that epoch's signing policy exists, and it is learned at runtime from the policy's own `startVotingRoundId` (from the `SigningPolicyInitialized` event where a client listens for it, and read back from the systems manager otherwise, which stores it in the same transaction that emits the event). Both Relays are read for `SigningPolicyInitialized` and `ProtocolMessageRelayed` across the switch, since a reward epoch's policy is only emitted by the Relay that holds it. A finalization goes to the Relay storing its signing policy's hash, so a late pre-cutover round is still relayed to the old contract, and the already-relayed check is made per target: a round relayed only on the old Relay is not relayed for readers of the new one. The section ships unset — without it everything keeps the current behaviour, and a half-set schedule (either value without the other) is rejected at startup rather than silently disabling the switch.
- The random number and its Merkle proof are appended to a finalization from the breaking reward epoch on, as the new Relay's `relay()` requires for the random protocol: `randomNumber(32)` followed by the proof nodes, after the signatures. The old Relay derived the random from the signed Merkle root instead, so nothing was appended and no source was needed. The data comes from the random protocol's own data provider — only it can produce the number and its proof — which serves it with its `submitSignatures` message, as a new `finalizationData` field of that response: a `0x` hex string of those bytes, the value followed by the proof nodes. The submitter forwards the field to the finalizer with the message it already hands over, so the duty needs no new configuration and no new endpoint, and the data exists exactly when the message does — the proof is a leaf of the tree the message's root commits to. The finalizer knows which protocol must carry it (the one whose id the Relay's `stateData` reports) and from which round on (the breaking epoch's first). On arrival it keeps only what `relay()` can read — a non-empty whole number of 32-byte words — logging an error naming the protocol and the round for a provider that omitted or truncated it, and it stores the bytes with the collection of the message that carried them, so they can never ride a competing Merkle root. What the bytes mean is the Relay's to check, and it does: it rebuilds the leaf from the voting round and the secure flag of the signed message, folds the proof, and reverts the whole `relay()` unless the result is the signed root. A round for which no usable data arrived is not sent at all, since the transaction could only revert, and the delayed queue retries it later. Nothing is expected until a Relay switch is scheduled for the chain; from then on the finalizer refuses to start unless that protocol is configured, since the data can only arrive through it.
...
- A prolonged reward epoch's finalization threshold is computed the way the Relay computes it — the signing policy's own threshold scaled by the Relay's `thresholdIncreaseBIPS`, read from the `stateData` call the client already makes at startup — instead of a hardcoded 60% of total weight. The old formula sat one weight unit below the contract's for every odd total weight (odd on Coston and Coston2 today), and since both acceptance tests are strict it could select exactly the contract's threshold and be rejected with `NotEnoughWeight`; at any `thresholdIncreaseBIPS` other than the deployed 12000 it diverged further, and at 15000 every prolonged-epoch finalization would have been sub-threshold fleet-wide. Not gated on the cutover: both Relays scale the policy threshold with the same arithmetic, so the old formula was wrong against the currently deployed Relay too.
- Protocol-message signatures bind the source chain from the Relay cutover on: the digest becomes `keccak256(chainID ‖ message)` instead of `keccak256(message)`, so a signature minted for another network is rejected even under a fully overlapping voter set. Signing (submitter) and signer recovery (finalizer) pick the digest form by the voting round embedded in the message bytes — the contract's own derivation, so any two components hashing the same bytes agree — through one shared function and one cutover instance, so the two gates cannot drift apart. While the breaking epoch's start round is still unknown the round cannot be past it, so the pre-switch digest is used; the finalizer, which after a restart can hold a policy without having seen the boundary, then falls back to deciding by the collection's signing-policy reward epoch.
- The signing-policy hash signed in `signNewSigningPolicy` follows the cutover: the old Relay's chained keccak fold before the breaking reward epoch, one keccak over the chain id and the raw policy bytes from it on. Which Relay the FlareSystemsManager points at is part of that decision rather than a substitute for it — the new Relay delegates epochs below the breaking one to the old Relay, and until governance repoints the manager the old Relay answers for every epoch, including the breaking one, whose policy is signed before a repoint is possible. The other scheme stays a checked fallback that logs a warning naming the epoch and the Relay, so a cutover schedule disagreeing with the chain costs a warning instead of the signature; the manager accumulates signing weight per epoch rather than per hash, so a mixed-scheme fleet still reaches one threshold. Only a hash derived from the policy bytes carried by the `SigningPolicyInitialized` event is ever signed.
...
- Relay reverts are decoded when the contract raises a custom error instead of `Error(string)`. The new Relay's `relay()` is hand-written assembly that reverts with a bare 4-byte selector, so all of its reverts reached the client as an unknown error signature, and the already-relayed case — a round another finalizer got to first — stopped being recognized as the non-fatal outcome it is: a delayed finalization, which is dry-run before it is sent, burned its whole send-retry budget and failed with an error log instead of one info line, and a duplicate reconciled after `nonce too low` was reported as conclusively reverted rather than accepted. The selectors are derived from the error names `IRelay` declares, so a decoded reason is the Solidity signature (`AlreadyRelayed()`), and both spellings count as non-fatal: only the old Relay can emit `Already relayed` and only the new one the custom error, so a round's own contract selects which applies, and every reward epoch before the cutover keeps matching the string exactly as before. A reverting `eth_estimateGas`, whose message is a bare `execution reverted` with the reason only in its data, now carries the decoded reason appended — appended rather than substituted, since the registration and uptime duties match the node's own text. A selector the client does not know stays undecodable and is still treated as a deterministic revert.
