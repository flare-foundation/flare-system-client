...
func NewClient(ctx clientContext.ClientContext, messageChannel chan<- shared.ProtocolMessage, relayCutover *shared.RelayCutover) (*client, error) {
	...
	if cfg.Submit1.Enabled {
		pc.submitter1 = newSubmitter(cl, chainCfg.ChainID, protocolContext, votingRoundTiming,
	...
	if cfg.Submit2.Enabled {
		pc.submitter2 = newSubmitter(cl, chainCfg.ChainID, protocolContext, votingRoundTiming,
...
func (c *client) isRegistered(ctx context.Context, rewardEpoch int64) (bool, error) {
	...
	err := backoff.RetryNotify(
		func() (err error) {
			...
			registered, err = c.registry.IsVoterRegistered(ctx, c.identityAddress, rewardEpoch)
