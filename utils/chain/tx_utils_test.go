...
func TestSendTx(t *testing.T) {
	...
	err = chain.SendRawTx(cl, pk, nonce, toAddress, []byte{1, 2, 3}, true, &gasConfigType2, 4*time.Second)
	require.NoError(t, err)

	gasConfigType0 := config2.Gas{TxType: 0, GasPriceMultiplier: 3}
	err = chain.SendRawTx(cl, pk, nonce+1, toAddress, []byte{1, 2, 3}, true, &gasConfigType0, 4*time.Second)
	require.NoError(t, err)
}
